 ...
 * ----------------------------------------------------------------------------
 *  Provenance: copied verbatim (algorithm-identical) from
 *    dashboard.drbastaninejad.com/public/assets/js/jalali.js
 *  to keep both frontends bit-for-bit consistent per REPOSITORY_AUDIT.md §1.7
 *  and SPACE_COORDINATION_PROTOCOL.md §6 (single algorithm authority).
 *
 *  Backend stores everything as UTC / Gregorian. Jalali is a display-only
 *  layer, per the non-negotiable Iranian market constraint in
 *  docs/ARCHITECTURE.md §7 and the design brief (Medical CRM.md §2.4/§6.7).
 *
 *  Old-browser note: uses `const`, arrow functions, and template literals.
 *  Every browser targeted by the CRM (last 2 majors + iOS Safari 15+, Android
 *  WebView 100+) supports these. The intake page — which also targets very
 *  old Persian-market devices — deliberately does NOT depend on this file;
 *  intake owns its own inline formatter in intake.html.
 * ============================================================================
 */
const Jalali = (() => {
