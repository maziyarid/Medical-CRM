...
final class GoogleCalendarClient
{
    ...
    public function listChanges(string $calendarId, ?string $syncToken): array
    {
        ...
        do {
            ...
            $response = $this->api('GET', '/calendar/v3/calendars/' . rawurlencode($calendarId) . '/events', null, $query);
            ...
            foreach (($response['body']['items'] ?? []) as $event) if (is_array($event)) $events[] = $event;
    ...
    public function busyRanges(string $calendarId, string $timeMinRfc3339, string $timeMaxRfc3339): array
    {
        if ($this->bridgeConfigured()) {
            $result = $this->bridge('/calendar/freebusy', [
                'calendar_id' => $calendarId,
                'time_min' => $timeMinRfc3339,
                'time_max' => $timeMaxRfc3339,
            ]);
            if (empty($result['ok'])) throw new RuntimeException('Google Calendar free/busy failed');
            $busy = [];
            foreach (($result['busy'] ?? []) as $row) {
                if (is_array($row) && !empty($row['start']) && !empty($row['end'])) {
                    $busy[] = ['start' => (string)$row['start'], 'end' => (string)$row['end']];
                }
            }
            return $busy;
        }
        $response = $this->api('POST', '/calendar/v3/freeBusy', [
        ...
        if ($response['status'] < 200 || $response['status'] >= 300) {
            throw new RuntimeException('Google Calendar free/busy failed: HTTP ' . $response['status']);
        }
        $calendar = $response['body']['calendars'][$calendarId] ?? [];
        if (!empty($calendar['errors'])) throw new RuntimeException('Google Calendar free/busy returned errors');
        $busy = [];
        foreach (($calendar['busy'] ?? []) as $row) {
            if (is_array($row) && !empty($row['start']) && !empty($row['end'])) {
                $busy[] = ['start' => (string)$row['start'], 'end' => (string)$row['end']];
            }
        }
    ...
    private function bridge(string $path, array $body): array
    {
        ...
        $ch = curl_init($url);
        curl_setopt_array($ch, [CURLOPT_POST=>true,CURLOPT_POSTFIELDS=>$json,CURLOPT_HTTPHEADER=>['Content-Type: application/json','Accept: application/json','Authorization: Bearer '.$token],CURLOPT_RETURNTRANSFER=>true,CURLOPT_FOLLOWLOCATION=>false,CURLOPT_CONNECTTIMEOUT=>3,CURLOPT_TIMEOUT=>20]);
        $raw = curl_exec($ch); $status = (int)curl_getinfo($ch, CURLINFO_RESPONSE_CODE); $error = curl_error($ch); curl_close($ch);
        if ($raw === false || $status < 200 || $status >= 300) throw new RuntimeException('Google bridge request failed' . ($error ? ': '.$error : ' HTTP '.$status));
    ...
    private function api(string $method, string $path, ?array $body = null, array $query = [], array $extraHeaders = []): array
    {
        ...
        $headers = array_merge(['Authorization: Bearer ' . $this->token(),'Accept: application/json'], $extraHeaders);
    ...
    private function token(): string
    {
        ...
        if ($clientId === '' || $clientSecret === '' || $refreshToken === '') throw new RuntimeException('Google Calendar OAuth is not configured');
        if (!function_exists('curl_init')) throw new RuntimeException('cURL extension is required for Google Calendar');
        $ch = curl_init('https://oauth2.googleapis.com/token');
        curl_setopt_array($ch, [CURLOPT_POST=>true,CURLOPT_POSTFIELDS=>http_build_query(['client_id'=>$clientId,'client_secret'=>$clientSecret,'refresh_token'=>$refreshToken,'grant_type'=>'refresh_token'],'','&',PHP_QUERY_RFC3986),CURLOPT_HTTPHEADER=>['Content-Type: application/x-www-form-urlencoded','Accept: application/json'],CURLOPT_RETURNTRANSFER=>true,CURLOPT_CONNECTTIMEOUT=>8,CURLOPT_TIMEOUT=>20]);
        $raw=curl_exec($ch);$status=(int)curl_getinfo($ch,CURLINFO_RESPONSE_CODE);$error=curl_error($ch);curl_close($ch);
        $decoded=is_string($raw)?json_decode($raw,true):null;$token=is_array($decoded)?trim((string)($decoded['access_token']??'')):'';
        if($status<200||$status>=300||$token==='')throw new RuntimeException('Google OAuth refresh failed'.($error?': '.$error:''));
    ...
    private function curlJson(string $method, string $url, ?array $body, array $headers): array
    {
        if (!function_exists('curl_init')) throw new RuntimeException('cURL extension is required for Google Calendar');
        $ch=curl_init($url);
        if($body!==null){$encoded=json_encode($body,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_THROW_ON_ERROR);$headers[]='Content-Type: application/json';curl_setopt($ch,CURLOPT_POSTFIELDS,$encoded);}
        curl_setopt_array($ch,[CURLOPT_CUSTOMREQUEST=>$method,CURLOPT_HTTPHEADER=>$headers,CURLOPT_RETURNTRANSFER=>true,CURLOPT_FOLLOWLOCATION=>false,CURLOPT_CONNECTTIMEOUT=>8,CURLOPT_TIMEOUT=>25]);
        $raw=curl_exec($ch);$status=(int)curl_getinfo($ch,CURLINFO_RESPONSE_CODE);$errno=curl_errno($ch);$error=curl_error($ch);curl_close($ch);
        if($raw===false||$errno!==0)throw new RuntimeException('Google Calendar transport failed: '.$error);
