...
final class StaffAccountService
{
    ...
    public function list(int $clinicId): array
    {
        ...
        foreach ($rows as &$row) {
            ...
            $row['activation_status'] = ($row['has_password'] || !empty($row['last_login_at'])) ? 'active' : 'invited';
        }
        unset($row);
        return $rows;
    }
    ...
    public function update(int $clinicId, int $actorId, int $userId, array $input): array
    {
        ...
        try {
            ...
            if (($roles[0] ?? '') !== $role || count($roles) !== 1) {
                $r = $db->prepare('SELECT id FROM roles WHERE name=? LIMIT 1');
                $r->execute([$role]);
                $rid = (int)$r->fetchColumn();
    ...
    public function bootstrapFirstSuperAdmin(int $clinicId, string $name, string $mobile, string $email=''): array
    {
        ...
        try {
            ...
            if (!ValidatorService::isValidMobile($mobile)) throw new RuntimeException('invalid staff mobile');
            if ($email !== '' && !filter_var($email, FILTER_VALIDATE_EMAIL)) throw new RuntimeException('invalid staff email');
            $exists = $db->prepare('SELECT id FROM users WHERE mobile=? AND deleted_at IS NULL LIMIT 1 FOR UPDATE');
            $exists->execute([$mobile]);
            if ($exists->fetchColumn()) throw new RuntimeException('staff mobile already registered');
            $r = $db->query("SELECT id FROM roles WHERE name='super_admin' LIMIT 1");
            $rid = (int)$r->fetchColumn();
            if ($rid < 1) throw new RuntimeException('super admin role is not configured');
            $uuid = bin2hex(random_bytes(16));
            $db->prepare(
                'INSERT INTO users (uuid,clinic_id,full_name,mobile,email,invited_at,activated_at,is_active,created_at,updated_at)
    ...
    {
        $db = Database::conn();
        $q = $db->prepare('SELECT full_name,mobile,email,password_hash FROM users WHERE id=? LIMIT 1');
        $q->execute([$userId]);
        $u = $q->fetch();
        if (!$u) return ['sms'=>'failed','email'=>'skipped'];

        $loginUrl = trim((string)($_ENV['STAFF_LOGIN_URL'] ?? 'https://app.drbastaninejad.com/Frontend/pages/auth/login.html'));
        $destination = $loginUrl;
        $needsActivation = empty($u['password_hash']);
        if ($needsActivation) {
            $activationBase = trim((string)($_ENV['STAFF_ACTIVATION_URL'] ?? 'https://app.drbastaninejad.com/Frontend/pages/auth/staff-activate.html'));
            $activation = (new PasswordAuthService())->issueStaffActivationToken($userId, 86400);
            $destination = $activationBase . (str_contains($activationBase, '?') ? '&' : '?') . 'token=' . rawurlencode((string)$activation['token']);
        }

        $text = $needsActivation
            ? 'حساب کارکنان کلینیک دکتر شاهین باستانی‌نژاد برای شما ایجاد شده است. برای فعال‌سازی و ساخت رمز عبور از این لینک یکبارمصرف استفاده کنید: ' . $destination
            : 'حساب کارکنان کلینیک دکتر شاهین باستانی‌نژاد فعال است. برای ورود: ' . $loginUrl;
        $sms = (new SmsProviderChain())->sendMessage((string)$u['mobile'], mb_substr($text, 0, 800));
        $smsStatus = !empty($sms['ok']) ? 'sent' : 'failed';
        $emailStatus = 'skipped';
        if (!empty($u['email'])) {
            $emailStatus = (new EmailService())->sendStaffInvitation((string)$u['email'], (string)$u['full_name'], $destination, $needsActivation) ? 'sent' : 'failed';
        }
