...
final class AuthController extends Controller
{
    public function passwordLogin(Request $req): array
    {
        ...
        try {
            ...
            if ($e->getMessage()==='too many login attempts') return $this->error('تلاش‌های ورود بیش از حد مجاز است. ۱۵ دقیقه بعد دوباره تلاش کنید.',429);
            if ($e->getMessage()==='staff schema migration lock timeout') return $this->error('سامانه در حال آماده‌سازی است. دوباره تلاش کنید.',503);
            return $this->error('شماره همراه یا رمز عبور نادرست است.',401);
        }
    }
    ...
    {
        $user=$req->user??[];
        if (($user['user_type']??'')!=='staff') return $this->error('این عملیات فقط برای کارکنان مجاز است.',403);
    ...
    public function logout(Request $req): array
    {
        ...
        if ($tokenId > 0) (new OtpService())->revokeTokenById($tokenId);
    ...
    public function logoutAll(Request $req): array
    {
        ...
        if ($userId < 1 || !in_array($userType, ['patient', 'staff'], true)) return $this->error('نشست نامعتبر است.', 401);
