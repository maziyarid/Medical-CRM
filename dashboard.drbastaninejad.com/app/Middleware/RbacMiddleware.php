 ...
 *  - 'patient' role only passes permission checks named 'patient.*'.
...
final class RbacMiddleware
{
    ...
    public function handle(Request $req): ?array
    {
        ...
        if (!$user) {
            ...
            return $this->forbidden('احراز هویت انجام نشده است');
        }
        ...
        if ($user['user_type'] === 'patient') {
            if (!str_starts_with($this->requiredPermission, 'patient.')) {
                return $this->forbidden('دسترسی شما به این بخش مجاز نیست');
            }
    ...
    {
        return ['ok' => false, 'status' => 403, 'data' => null, 'errors' => [['field' => null, 'message' => $message]]];
    }
}
