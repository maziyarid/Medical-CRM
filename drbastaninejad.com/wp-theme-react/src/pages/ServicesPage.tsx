...
import { COSMETIC_SERVICES, FUNCTIONAL_SERVICES, BOOKING_URL } from "@/data/site";
import Breadcrumb from "@/components/Breadcrumb";

export default function ServicesPage() {
  return (
    <>
      <Breadcrumb items={[{ label: "خانه", href: "/" }, { label: "همه خدمات" }]} />
            ...
            {COSMETIC_SERVICES.map(s => {
              ...
              return (
                <Link key={s.id} to={`/services/${s.slug}`}
                  ...
                  <div className="w-12 h-12 bg-[#E4F0E4] rounded-2xl flex items-center justify-center mb-4 group-hover:bg-[#28722C] transition-colors">
                    <Icon className="w-6 h-6 text-[#28722C] group-hover:text-white transition-colors" />
                  </div>
                  <h3 className="text-lg font-bold text-[#25272C] mb-1">{s.title}</h3>
                  <p className="text-sm text-[#28722C] font-medium mb-3">{s.subtitle}</p>
                  <p className="text-sm text-[#6A7078] leading-relaxed line-clamp-3">{s.desc}</p>
