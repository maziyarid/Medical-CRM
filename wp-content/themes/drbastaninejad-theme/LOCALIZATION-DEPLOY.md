...
For Arabic specifically, record the first visible navigation frame and open/close both desktop and mobile menus repeatedly. No Persian label should flash and already Arabic labels should not be rewritten during menu animation.
...
For Arabic, verify the canonical navigation wording: `الرئيسية`, `عن الطبيب`, `جراحة الأنف`, `المعرض`, `المقالات`, `الأسئلة الشائعة`, `تواصل معنا`.
...
Where the booking form shows a separate `+98` dial-code field, enter only the national number in the phone input. Confirm `+98` is serialized exactly once.
