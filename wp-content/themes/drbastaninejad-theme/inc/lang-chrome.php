...
if ( ! function_exists( 'drb_language_dropdown_html' ) ) {
	...
	function drb_language_dropdown_html(): string {
		...
		$instance++;
		$menu_id = 'drb-language-menu-' . $instance;

		ob_start();
		?>
		<div class="drb-langdd" data-drb-langdd data-drb-no-translate>
			<button type="button"
			...
			<ul id="<?php echo esc_attr( $menu_id ); ?>" class="drb-langdd__menu" aria-hidden="true" inert>
				<?php foreach ( $meta as $code => $row ) : ?>
					<?php
					$url    = drb_lang_url( $code );
					$active = ( $code === $current );
					?>
					<li class="drb-langdd__item<?php echo $active ? ' is-active' : ''; ?>">
						<a href="<?php echo esc_url( $url ); ?>" hreflang="<?php echo esc_attr( $code ); ?>" lang="<?php echo esc_attr( $code ); ?>"<?php echo $active ? ' aria-current="page"' : ''; ?>>
...
if ( ! function_exists( 'drb_force_html_dir_lang_js' ) ) {
	...
	function drb_force_html_dir_lang_js(): void {
		...
		$locale  = drb_detect_lang();
		$lang    = esc_js( function_exists( 'drb_html_lang' ) ? drb_html_lang() : 'fa-IR' );
		$dir     = esc_js( ( function_exists( 'drb_is_rtl' ) && drb_is_rtl() ) ? 'rtl' : 'ltr' );
		$pending = 'fa' !== $locale ? 'h.classList.add("drb-i18n-pending");window.setTimeout(function(){h.classList.remove("drb-i18n-pending");},4000);' : '';
		echo '<script>(function(){var h=document.documentElement;if(h){h.setAttribute("lang","' . $lang . '");h.setAttribute("dir","' . $dir . '");' . $pending . '}})();</script>' . "\n";
	}
}
